...
class ToolSearcher(API):
    ...
    def __init__(self):
        ...
        def api_summery(cls):
            ...
            cls_name = "".join(
                [" " + i.lower() if i.isupper() else i for i in cls_name]
            ).strip()
    ...
    def check_api_call_correctness(self, response, groundtruth) -> bool:
        if response["exception"] != groundtruth["exception"]:
            return False
        try:
            pred_api_names = [out["name"] for out in response["output"]]
            gt_api_names = [out["name"] for out in groundtruth["output"]]
            return all(api_name in pred_api_names for api_name in gt_api_names)
        except:
            return False
