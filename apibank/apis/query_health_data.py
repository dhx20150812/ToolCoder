...
class QueryHealthData(API):
    ...
    def query_health_data(self, user_id, start_time, end_time) -> list:
        ...
        for health_data_item in self.database[user_id]:
            health_data_item_time = datetime.datetime.strptime(
                health_data_item["time"], "%Y-%m-%d %H:%M:%S"
            )
            if (
                health_data_item_time >= start_time
                and health_data_item_time <= end_time
            ):
    ...
    def check_api_call_correctness(self, response, groundtruth) -> bool:
        ...
        response_user_id, response_start_time, response_end_time = self.format_check(
            response_user_id, response_start_time, response_end_time
        )
        groundtruth_user_id, groundtruth_start_time, groundtruth_end_time = (
            self.format_check(
                groundtruth_user_id, groundtruth_start_time, groundtruth_end_time
            )
        )
