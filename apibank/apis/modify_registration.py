...
class ModifyRegistration(API):
    ...
    def call(
        ...
        new_appointment_doctor: str = None,
    ) -> dict:
        """
        ...
        try:
            status = self.modify_registration(
                appointment_id, new_appointment_date, new_appointment_doctor
            )
    ...
    def modify_registration(
        ...
        if new_appointment_date == None:
            ...
            if new_appointment_doctor == None:
                ...
                if (
                    new_appointment_date == self.database[appointment_id]["date"]
                    and new_appointment_doctor
                    == self.database[appointment_id]["doctor_name"]
                ):
                    raise Exception("Fail because no modification is made.")
                if new_appointment_date == self.database[appointment_id]["date"]:
                    raise Exception("Fail because date is not changed.")
    ...
    def check_api_call_correctness(self, response, groundtruth) -> bool:
        ...
        groundtruth_new_appointment_doctor = groundtruth["input"][
            "new_appointment_doctor"
        ]
        ...
        if response_new_appointment_date != None:
            response_new_appointment_date = self.format_check(
                response_new_appointment_date
            )
        if groundtruth_new_appointment_date != None:
            groundtruth_new_appointment_date = self.format_check(
                groundtruth_new_appointment_date
            )
        ...
        if groundtruth_new_appointment_doctor != None:
            groundtruth_new_appointment_doctor = (
                groundtruth_new_appointment_doctor.strip()
            )
