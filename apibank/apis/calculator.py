...
class Calculator(API):
    ...
    def is_valid_formula(self, formula: str) -> bool:
        ...
        if formula.count("+") + formula.count("-") + formula.count("*") + formula.count(
            "/"
        ) + 1 == len(formula):
            return False
    ...
    def calculate_formula(self, formula: list) -> float:
        ...
        while "(" in formula:
            ...
            formula[left_parenthesis_index : right_parenthesis_index + 1] = [
                self.calculate_formula(
                    formula[left_parenthesis_index + 1 : right_parenthesis_index]
                )
            ]
        ...
        while "*" in formula or "/" in formula:
            ...
            formula[index - 1 : index + 2] = [
                self.calculate_operation(
                    formula[index - 1], formula[index], formula[index + 1]
                )
            ]
