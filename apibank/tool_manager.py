...
class ToolManager:
    def __init__(self, apis_dir="./apis") -> None:
        ...
        for cls in classes:
            if issubclass(cls, object) and cls is not object:
                ...
                if (
                    hasattr(cls, "database_name")
                    and cls.database_name in self.init_databases
                ):
    ...
    def init_tool(self, tool_name: str, *args, **kwargs):
        ...
        if (
            ...
            and "token" in self.get_api_by_name(tool_name)["input_parameters"]
        ):
    ...
    def process_parameters(self, tool_name: str, parameters: list):
        ...
        for this_para, input_para in zip(parameters, input_parameters):
            ...
            if para_type == "int":
                assert (
                    this_para.isdigit()
                ), "invalid parameter type. parameter: {}".format(this_para)
                processed_parameters.append(int(this_para))
            elif para_type == "float":
                assert this_para.replace(
                    ".", "", 1
                ).isdigit(), "invalid parameter type."
    ...
    def api_call(self, tool_name: str, **kwargs):
        ...
        """
        input_parameters = self.get_api_by_name(tool_name)[
            "input_parameters"
        ]  # {'username': {'type': 'str', 'description': 'The username of the user.'}, 'password': {'type': 'str', 'description': 'The password of the user.'}}
        ...
        for input_key in kwargs:
            ...
            ), "invalid parameter name. parameter: {}".format(input_key)
            ...
            if required_type == "int":
                if isinstance(input_value, str):
                    assert (
                        input_value.isdigit()
                    ), "invalid parameter type. parameter: {}".format(input_value)
                processed_parameters[input_key] = int(input_value)
            elif required_type == "float":
                if isinstance(input_value, str):
                    assert input_value.replace(
                        ".", "", 1
                    ).isdigit(), "invalid parameter type."
    ...
    def command_line(self):
        ...
        if mode == "qa":
            while True:
                ...
                while True:
                    command = input(
                        "Please enter the parameters for the tool you want to use ('exit' to exit): \n"
                    )
                    if command == "exit":
                        break
                    else:
                        command = command.replace(" ", "")
                        processed_parameters = self.process_parameters(
                            response["output"]["name"], command.split(",")
                        )
                        print(tool.call(*processed_parameters))
        elif mode == "function_call":
            while True:
                command = input(
                    "Please enter the command for the tool you want to use: \n"
                )
                if command == "exit":
                    break
